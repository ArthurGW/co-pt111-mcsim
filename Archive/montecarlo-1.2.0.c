...
/* Update notes: Added small timesteps option							*/
	...
	do {	/* Main loop (over repeats) */
		...
		for(time_index=0;time_index<time_steps+1;time_index++) {
			...
			for(i=0;i<dim/2+1;i++) {	/* Transfer results from temp. array to ISF average array */
				ISF_av[time_index*(dim+2)+2*i][0]+=temp_ISF[i][0]/(n_co*repeats);
				ISF_av[time_index*(dim+2)+2*i][1]+=temp_ISF[i][1]/(n_co*repeats);
				ISF_av[time_index*(dim+2)+2*i+1][0]+=(temp_ISF[i][0]*temp_ISF[i][0])/(n_co*n_co*repeats);
				ISF_av[time_index*(dim+2)+2*i+1][1]+=(temp_ISF[i][1]*temp_ISF[i][1])/(n_co*n_co*repeats);
			}		
		}
...
{							/* try a different file, then another, then fails 						*/
	...
	if( out_num==1 ) {	/* Header section */
		...
		for(j=0;j<(time_steps+1);j++) {
			fprintf(tmp,"%d\t%d\t",0,s?(int)j*time_factor:(int)pow(time_factor,j));
			for(i=0;i<dim+2;i+=2)
				fprintf(tmp,"%.5f\t",0.0142*sqrt(ISF_av[j*(dim+2)+i+1][0]));
			fprintf(tmp,"\n");
		}
		fprintf(tmp,"\nImaginary variance\nTimestep 1\tTimestep 2\t");
		for(i=0;i<dim/2+1;i++) fprintf(tmp,"k%d\t",i);
		...
		for(j=0;j<(time_steps+1);j++) {
			fprintf(tmp,"%d\t%d\t",0,s?(int)j*time_factor:(int)pow(time_factor,j));
			for(i=0;i<dim+2;i+=2)
				fprintf(tmp,"%.5f\t",0.0142*sqrt(ISF_av[j*(dim+2)+i+1][1]));
			fprintf(tmp,"\n");
		}
