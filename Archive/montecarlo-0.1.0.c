...
/* 												*/
/* Version number 0.1.0, date 27/11/09								*/
/* Update notes: Added output function, file test function					*/
...
{					/* Returns 0 if successful */
	...
	if( int_mode<1 || int_mode>3 ) {
		printf("Please enter an interaction mode between 1 and 3.\n");
		return 2;	
	}
...
{				/* try a different file, then another, then fails 				*/
	...
	if( out_num==1 ) {	/* Header section */
		...
		for(i=0;i<bins;i++)
			if( G[i] )
				fprintf(tmp,"%2.7f\t%1.9f\t%d\n",i*bin_res,(float)G[i]/normalisation,G[i]);
...
int sweep ( )
{
	int i,j,k,lineindex,atom,coord,move_coord,nn[6],nns,mnn;
	...
	for(i=0;i<n_co;i++) {
		...
		get_nns(coord,nn);	/* Populate nearest neighbour coordinate matrix */
