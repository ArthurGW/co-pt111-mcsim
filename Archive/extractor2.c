...
{
	int i,j,k, reps[11]={0,100,200,300,400,500,600,700,800,900,1000};
	...
	sprintf(outname,"seriesreps.csv2");
	out=fopen(outname,"w");

	for(i=0;i<5;i++) {
		fprintf(out,"%.3f",thetas[i]);
		sprintf(zeroname,"32-%.3f-0.csv2",thetas[i]);
		printf("zeroname: %s\n",zeroname);
		...
		for(j=0;j<11;j++){
			meansquare[0]=0.;
			meansquare[1]=0.;
			sprintf(inname,"32-%.3f-%d.csv2",thetas[i],reps[j]);
			printf("inname: %s\n",inname);
			in=fopen(inname,"r");
			for(k=0;k<(dim/2);k++) {
				buffer=(char *)malloc(100*sizeof(char));
