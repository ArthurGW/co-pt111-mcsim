...
/* 												*/
/* Version number 1.2.0, date 17/12/09								*/
/* Update notes: Exponentiated probabilities							*/
...
time_t start_time, total_sweeps=0, count=0, rep=10;
double bin_res;
...
int main ( int argc, char *argv[] )
{
	...
	fft_steps=(fftw_complex *) fftw_malloc( (time_steps+1) * 2 * sizeof(double) * dim/2 );
	...
	fftw_free(temp_ISF); fftw_free(fft_steps); free(adatoms); fftw_free(temp_occs); /* Free arrays that are no longer needed */
...
{							/* try a different file, then another, then fails 						*/
	...
	static char tmp_name[15],ISF_out_name[20],param_out_name[20];
	char * buffer;
	...
	if( out_num==1 ) {	/* Header section */
		...
		sprintf(ISF_out_name,"%d-%.3f-%d.ISF",dim,n_co/n_sites,int_mode?rep:0);
		sprintf(param_out_name,"%d-%.3f-%d.prm",dim,n_co/n_sites,int_mode?rep:0);
		...
		for(i=0;i<dim/2;i++) {
			fprintf(param_out,"m%d = %.5f\nc%d = %.5f",i,log(ISF_av[i][0]/ISF_av[(count[i]-1)*dim+i][0])/(s?TIMESCALE*(count[i]-1)*time_factor:TIMESCALE*pow(time_factor,(count[i]-1))),i,ISF_av[i][0]);
		}
		fclose(param_out);
